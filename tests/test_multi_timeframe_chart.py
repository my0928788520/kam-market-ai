...
def test_chart_page_shows_session_badges_from_source() -> None:
    ...
    assert night_html.count("action='/session-switch'") == 2
    assert "name='session' value='regular'" in night_html
    assert "name='session' value='afterhours'" in night_html
    assert "name='instrument' value='TMF'" in night_html
    assert "name='timeframe' value='60m'" in night_html
    assert "view_session=regular" not in night_html
    assert "view_session=afterhours" not in night_html
