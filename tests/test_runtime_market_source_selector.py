...
from kam_market_ai.live_read_only.runtime_market_source import RuntimeMarketSourceConfig, RuntimeMarketSourceError, RuntimeMarketSourceMode, RuntimeMarketSourceSelector

def test_default_and_fake_live_are_explicit_and_read_only():
    selector=RuntimeMarketSourceSelector(); offline=selector.select(); fake=selector.select(RuntimeMarketSourceConfig(RuntimeMarketSourceMode.FAKE_LIVE))
...
def test_reserved_and_invalid_fail_closed():
    reserved=RuntimeMarketSourceSelector().select(RuntimeMarketSourceConfig(RuntimeMarketSourceMode.FUGLE_LIVE_RESERVED))
    assert reserved.status.value=="RESERVED" and reserved.provider.list_available_products()==()
    with pytest.raises(RuntimeMarketSourceError): RuntimeMarketSourceSelector().select(RuntimeMarketSourceConfig("bad")) # type: ignore[arg-type]
...
def test_reserved_runtime_source_renders_fail_closed_header_and_preserves_drawer_safety():
    ...
    reserved = RuntimeMarketSourceSelector().select(RuntimeMarketSourceConfig(RuntimeMarketSourceMode.FUGLE_LIVE_RESERVED)).provider
    app = build_operator_wsgi(lambda: PaperTradingOperatorView("KAM", "", {}, {}, {"cash": "0"}, (), False), market_data_source=reserved)
    html = b"".join(app({"REQUEST_METHOD": "GET", "PATH_INFO": "/", "QUERY_STRING": ""}, lambda *_: None)).decode("utf-8")
    for text in ("真實行情來源尚未啟用", "資料不足／無法判讀", "帳戶未連線", "券商未連線", "唯讀模式", "禁止真實下單"):
        assert text in html
...
def _operator_html_for(mode: RuntimeMarketSourceMode) -> str:
    ...
    return b"".join(app({"REQUEST_METHOD": "GET", "PATH_INFO": "/", "QUERY_STRING": ""}, lambda *_: None)).decode("utf-8")
...
def test_cli_import_and_app_composition_do_not_connect_or_start_background_threads(monkeypatch):
    ...
    monkeypatch.setattr(socket, "create_connection", lambda *args, **kwargs: pytest.fail("socket connect"))
    monkeypatch.setattr(threading.Thread, "start", lambda *args, **kwargs: pytest.fail("thread start"))
    module = _load_operator_cli_module()
...
def test_cli_passes_selected_market_source_to_every_app_composition(monkeypatch, flag):
    module = _load_operator_cli_module()
    seen = []
    monkeypatch.setattr(module, "create_operator_app", lambda view, *, market_data_source: seen.append(market_data_source) or (lambda *_: ()))
    if flag == "--demo":
        from kam_market_ai.paper_trading import operator_app
        monkeypatch.setattr(operator_app, "create_demo_operator_app", lambda *, market_data_source: seen.append(market_data_source) or (lambda *_: ()))
    elif flag == "--kam-rule-demo":
        from kam_market_ai.paper_trading import operator_app
        monkeypatch.setattr(operator_app, "create_kam_rule_demo_operator_app", lambda *, market_data_source: seen.append(market_data_source) or (lambda *_: ()))
    args = module.build_parser().parse_args(([flag] if flag else []) + ["--market-source", "offline-demo"])
