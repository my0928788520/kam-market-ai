...
from .providers.fubon_futures_runtime import FubonFuturesLiveClient
from .providers.fugle_futures_client import (
    DEFAULT_FUGLE_FUTURES_SYMBOL_REGISTRY,
    FugleFuturesPayloadMapper,
)
from .providers.fugle_futures_websocket_lifecycle import (
...
_DEFAULT_RUNTIME_SOURCE_CONFIG = RuntimeMarketSourceConfig()
...
class RuntimeMarketDataProvider:
    ...
    def read_snapshot(self, product_code: str) -> MarketSnapshot:
        return (
            self.source.read_snapshot(product_code)
            if self.status is RuntimeMarketSourceStatus.READY
            else self.source.read_snapshot("__RUNTIME_UNAVAILABLE__")
        )

    def list_available_products(self) -> tuple[str, ...]:
        return (
            self.source.list_available_products()
            if self.status is RuntimeMarketSourceStatus.READY
            else ()
        )
...
class _LifecycleCacheProvider(RuntimeMarketDataProvider):
    ...
    def read_snapshot(self, product_code: str) -> MarketSnapshot:
        ...
        return LiveMarketDataAdapter(
            FakeLiveMarketDataClient(()) if record is None else FakeLiveMarketDataClient((record,)),
            LiveMarketAdapterConfig("fake-live-runtime"),
        ).read_snapshot(product_code)

    def list_available_products(self) -> tuple[str, ...]:
        return self.lifecycle.list_ready_products()


class _FubonLiveProvider(RuntimeMarketDataProvider):
    client: FubonFuturesLiveClient

    def __init__(self, client: FubonFuturesLiveClient) -> None:
        adapter = LiveMarketDataAdapter(
            client,
            LiveMarketAdapterConfig(
                "fubon-neo-futures-live",
                source_selection=MarketSourceSelection.FUTURE_LIVE,
                stale_after_seconds=60,
                expire_after_seconds=300,
                data_source=MarketDataSource.FUTURE_LIVE,
            ),
        )
        object.__setattr__(self, "source", adapter)
        object.__setattr__(self, "mode", RuntimeMarketSourceMode.FUBON_LIVE)
        object.__setattr__(self, "status", RuntimeMarketSourceStatus.READY)
        object.__setattr__(self, "client", client)

    def runtime_status(self) -> RuntimeMarketSourceStatus:
        return (
            RuntimeMarketSourceStatus.READY
            if self.client.connection_ready
            else RuntimeMarketSourceStatus.DEGRADED
        )
...
class RuntimeMarketSourceSelector:
    def select(
        self,
        config: RuntimeMarketSourceConfig = _DEFAULT_RUNTIME_SOURCE_CONFIG,
        *,
        fubon_live_client: FubonFuturesLiveClient | None = None,
    ) -> RuntimeMarketSourceSelection:
        if config.mode is RuntimeMarketSourceMode.OFFLINE_DEMO:
            provider = RuntimeMarketDataProvider(
                OFFLINE_DEMO_MARKET_DATA_SOURCE, config.mode, RuntimeMarketSourceStatus.READY
            )
            return RuntimeMarketSourceSelection(
                provider,
                config.mode,
                RuntimeMarketSourceStatus.READY,
                "離線示範行情",
        ...
        if config.mode is RuntimeMarketSourceMode.FAKE_LIVE:
            now = __import__("datetime").datetime(
                2026, 8, 5, 9, 1, tzinfo=__import__("datetime").UTC
            )
            lifecycle = FugleFuturesWebSocketLifecycle(
                FugleFuturesWebSocketConfig(enabled=True),
                FakeFugleFuturesWebSocketTransport(),
                clock=lambda: now,
            )
            lifecycle.start()
            mapper = FugleFuturesPayloadMapper(
                DEFAULT_FUGLE_FUTURES_SYMBOL_REGISTRY, "fake-live-runtime"
            )
            for entry, price, volume in zip(
                DEFAULT_FUGLE_FUTURES_SYMBOL_REGISTRY.entries,
                ("24186", "24142", "24108"),
                ("14872", "39761", "82514"),
                strict=True,
            ):
                raw = {
                    ...
                    "timestamp": "2026-08-05T09:00:59+00:00",
                    "session": "DAY",
                    "market_status": "OPEN",
                }
        ...
        if config.mode is RuntimeMarketSourceMode.FUGLE_LIVE_RESERVED:
            provider = RuntimeMarketDataProvider(
                OFFLINE_DEMO_MARKET_DATA_SOURCE, config.mode, RuntimeMarketSourceStatus.RESERVED
            )
            return RuntimeMarketSourceSelection(
                provider,
                config.mode,
                RuntimeMarketSourceStatus.RESERVED,
                "真實行情來源尚未啟用",
                "LIVE_RESERVED",
            )
        if config.mode is RuntimeMarketSourceMode.FUBON_LIVE:
            if fubon_live_client is None or not fubon_live_client.connection_ready:
                raise RuntimeMarketSourceError("FUBON_LIVE_CLIENT_REQUIRED")
            provider = _FubonLiveProvider(fubon_live_client)
            return RuntimeMarketSourceSelection(
                provider,
                config.mode,
                RuntimeMarketSourceStatus.READY,
