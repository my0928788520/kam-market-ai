...
def _matching_rows(values: dict[str, str]) -> str:
    ...
    if "實盤狀態" in values and not any(key in values for key in _PERFORMANCE_KEYS):
        status["實盤狀態"] = values["實盤狀態"]
...
def render_operator_html(view: PaperTradingOperatorView) -> str:
    ...
    live_lock = (
        f"<span class='live-execution-lock'>{escape(str(view.matching['實盤狀態']))}</span>"
