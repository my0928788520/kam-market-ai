...
def create_demo_operator_app(market_data_source=None):
    ...
    return build_operator_wsgi(lambda: build_demo_operator_presenter(proposal, matching, DEMO_SNAPSHOT), market_data_source=market_data_source) if market_data_source is not None else build_operator_wsgi(lambda: build_demo_operator_presenter(proposal, matching, DEMO_SNAPSHOT))


def create_kam_rule_demo_operator_app(market_data_source=None):
    ...
    return build_operator_wsgi(lambda: build_demo_operator_presenter(proposal, None, DEMO_SNAPSHOT), market_data_source=market_data_source) if market_data_source is not None else build_operator_wsgi(lambda: build_demo_operator_presenter(proposal, None, DEMO_SNAPSHOT))


def create_operator_app(view_provider: Callable[[], PaperTradingOperatorView], market_data_source=None):
    """Return the GET-only WSGI app; no server is started here."""
    return build_operator_wsgi(view_provider, market_data_source=market_data_source) if market_data_source is not None else build_operator_wsgi(view_provider)
